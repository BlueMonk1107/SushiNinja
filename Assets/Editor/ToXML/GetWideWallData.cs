...
public class GetWideWallData : Editor
{
    private static float StartPoint = -31.33f;
    private static int _mark;
    [MenuItem("Assets/Export Scene WideWallData To XML From Selection")]
    static void ExportXML()
    {
        string path = EditorUtility.SaveFilePanel("Save Resource", "Assets/StreamingAssets", "WideWallData", "xml");
                ...
                foreach (GameObject sceneObject in FindObjectsOfType(typeof(GameObject)))
                {
                    ...
                    if (sceneObject.activeSelf)
                    {
                            ...
                            &&PrefabUtility.GetPrefabType(sceneObject) == PrefabType.PrefabInstance)
                        {
                            ...
                            if (prefabObject != null && prefabObject.name.Contains("Obj_zhangai02"))
                            {
